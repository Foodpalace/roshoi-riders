//#region node_modules/.nitro/vite/services/ssr/assets/catalog-CBGXwXS0.js
function maskPhone(phone) {
	const digits = phone.replace(/\D/g, "");
	if (digits.length < 4) return "••••";
	return `•••• ${digits.slice(-4)}`;
}
function displayNameFromFull(full) {
...
function minimizeCustomer(raw, state) {
	const pickedUp = state === "PICKED_UP" || state === "ON_THE_WAY" || state === "ARRIVED_AT_CUSTOMER" || state === "CUSTOMER_UNAVAILABLE" || state === "DELIVERED" || state === "SUPPORT_ESCALATION";
	const enRoute = state === "ON_THE_WAY" || state === "ARRIVED_AT_CUSTOMER" || state === "CUSTOMER_UNAVAILABLE" || state === "DELIVERED";
	return {
...
function offerCustomerView(raw) {
	return {
...
var RESTAURANTS = [
	{
		...
		phoneMasked: "•••• 2140",
		specialPickupInstructions: "Counter on the left. Ask for Roshoi bag.",
...
//#endregion
export { simulatedRestaurant as a, simulatedCustomer as i, minimizeCustomer as n, offerCustomerView as r, ZONES as t };
